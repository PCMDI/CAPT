...
This script interpolates ERA5 data to a E3SM ne120 grid. 
The output file(s) can then be used as nudging file(s) for nudging runs.
This should also be usalbe for CESM2 ne120(SE) grid.

Interactive usage:
    era52e3sm_ne120.py  <year month day hour
                         ntimes
...
#---------------------------------
    """
    Returns various E3SM ne120 horizontal coordinate variables.
    ...
    try:
        ...
        mystr = 'hybrid_file NOT FOUND IN _era52e3sm_ne120()!:\n    %s\n' % \
                hybrid_file
        raise Exception('\n  %s' % mystr)
...
#-----------------------------------
    """
    Returns various E3SM ne120 vertical hybrid coordinate variables.
    ...
    try:
        ...
        mystr = 'hybridv_file NOT FOUND IN _era52e3sm_ne120()!:\n    %s\n' % \
                hybridv_file
        raise Exception('\n  %s' % mystr)
    ...
    # p = p0*hyai + hybi*ps (E3SM ne120 model's version of era5 ak/bk).
    #------------------------------------------------------------------
...
#-----------------------------
    ...
    try:
        ...
        mystr = 'topo_file NOT FOUND IN _era52e3sm_ne120()!:\n    %s\n' % \
                topo_file
        raise Exception('\n  %s' % mystr)
...
#-------------------------------------------------------------------------
    """
    Sets up E3SM ne120 output file (same format as E3SM .i files);
    returns various CDMS variables.

    cbaset  # Starting time of data in E3SM ne120 output file.
    cendt   # Ending   time of data in E3SM ne120 output file.
    """
...
#-----------------------------------------
    ...
    try:
        ...
        mystr = 'map_file NOT FOUND IN _era52e3sm_ne120()!:\n    %s\n' % \
                map_file
        raise Exception('\n  %s' % mystr)
...
def _era52e3sm_ne120(year, month, day, hour, ntimes, state_xmlfile_dir,
...
#-------------------------
    ...
    if len(args) < 11:
        ...
        _era52e3sm_ne120(year, month, day, hour, ntimes, state_xmlfile_dir,
                         state_xmlfile_name, map_file_dir, map_file_name,
                         hybrid_file, topo_file, file_tag)
